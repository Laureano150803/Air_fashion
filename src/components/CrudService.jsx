...
const CrudService = () => {
  ...
  const filteredservicios = services.filter((service) => {
    const searchValue = filter.toLowerCase();
    return (
      service.nombre.toLowerCase().includes(searchValue) ||
      service.precio.toString().includes(searchValue) ||
      service.serviceTypeId.name.toLowerCase().includes(searchValue)
    );
  });
    ...
    <>
      <div className="w-full min-h-screen  ">
        ...
        <div className="flex justify-evenly items-center overflow-auto  mb-6">
          ...
          {openUpd && <UpdateService id={id} close={closeUpdModal} update={setUpdData} />}
